# @ohos-rs/ability-plugin-filepicker

这是文件选择对话框能力的 ArkTS HAR，对应 Rust crate `openharmony-ability-plugin-filepicker`。它在
...
```bash
ohpm install @ohos-rs/ability-plugin-filepicker
```
...
```json5
{
  "dependencies": {
    ...
    "@ohos-rs/ability-plugin-filepicker": "1.0.0-beta.1"
  }
}
```
...
Rust 侧还需注册 `FilePickerBridgePlugin`，并通过 `FilePickerExt::show_file_dialog` 发起调用。使用示例见
[Rust facade README](../../crates/plugin-filepicker/README.md)。
...
| request → response | `ohos.filepicker.DialogOptions` → `ohos.filepicker.DialogResponse` |
