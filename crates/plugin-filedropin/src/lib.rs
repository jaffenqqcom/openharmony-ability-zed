...
//!
//! The ArkTS `FileDropInPlugin` attaches a transparent drop-target overlay to the
...
/// Main-thread event names pushed by the ArkTS `FileDropInPlugin`.
...
impl_bridge_napi_type!(DragMoveData, "ohos.filedropin.DragMoveData");
...
impl_bridge_napi_type!(DropFilesData, "ohos.filedropin.DropFilesData");
...
impl_bridge_napi_type!(DropAck, "ohos.filedropin.DropAck");
...
thread_local! {
    static FILEDROPIN_CALLBACK: RefCell<Option<Box<dyn Fn(FileDropEventData)>>> = RefCell::new(None);
}

/// Registers the file-drop handler. Replaces any previously registered handler.
pub fn set_filedropin_callback(callback: Box<dyn Fn(FileDropEventData)>) {
    FILEDROPIN_CALLBACK.with(|cell| *cell.borrow_mut() = Some(callback));
}

/// Clears the file-drop handler. Called when the window that owns it is destroyed.
pub fn clear_filedropin_callback() {
    FILEDROPIN_CALLBACK.with(|cell| *cell.borrow_mut() = None);
}

pub struct FileDropInBridgePlugin;

impl BridgePlugin for FileDropInBridgePlugin {
    type Mode = AsyncBridge;

    const ID: &'static str = "ohos.filedropin";
        ...
        let data = match event.name() {
            ...
            other => {
                return Err(Error::from_reason(format!(
                    "Unsupported ohos.filedropin main-thread event '{other}'"
                )));
            }
        };
        if let Some(data) = data {
            FILEDROPIN_CALLBACK.with(|cell| {
