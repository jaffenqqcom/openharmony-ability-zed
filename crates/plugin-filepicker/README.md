# openharmony-ability-plugin-filepicker

`openharmony-ability-plugin-filepicker` 提供文件选择对话框（open / save / folder）的 Rust facade。它与
ArkTS HAR `@ohos-rs/ability-plugin-filepicker` 成对使用，是异步插件，可从 Rust worker 调用。

## 契约

| 项目 | 值 |
| --- | --- |
| Rust crate | `openharmony-ability-plugin-filepicker` |
| ArkTS HAR | `@ohos-rs/ability-plugin-filepicker` |
| 插件 ID | `ohos.filepicker` |
| 执行模式 | 异步：`AsyncBridge` / `invokeAsync` |
| 前置 context | `ability` |
| action | `file-dialog` |
| request → response | `ohos.filepicker.DialogOptions` → `ohos.filepicker.DialogResponse { files }` |
...
fn configure_ability(app: OpenHarmonyApp) {
    app.register_plugin(FilePickerBridgePlugin)
...
export default class EntryAbility extends NativeAbility {
  public bridgePlugins = [new LazyPlugin(() => new FilePickerPlugin())];
}
```

同时在应用 `oh-package.json5` 添加 `@ohos-rs/ability-plugin-filepicker`。HAR 的具体依赖和 plugin 说明见
[ArkTS README](../../plugins/filepicker/README.md)。

## Rust 使用方式

`FilePickerExt::show_file_dialog` 是异步 facade，可在 Rust worker 中调用。参数用 `FileDialogOptions`
